...
#inserção de notas referentes aos vinhos da adega, assim como mensagens de erro em caso não seja utilizada corretamente
...
#atualiza o dado referente ao ano dos vinhos na adega, assim como mensagens de erro em caso não seja utilizado corretamente
@app.put('/vinhoUpdate', tags=[vinho_tag],
          responses={"200": VinhoSchema, "404": ErrorSchema, "400": ErrorSchema})
def update_vinho(form: VinhoUpdateSchema):
    """Atualize o ano de um vinho na adega.
    """
    vinho_id = form.vinho_id
    new_vinho_year = form.new_vinho_year
    
    session = Session()
    vinho = session.query(Vinho).filter(Vinho.id == vinho_id).first()
    if not vinho:
        error_msg = "Vinho não encontrado na Adega :/"
        logger.warning(f"Erro ao atualizar ano do vinho '{vinho_id}', {error_msg}")
        return {"message": error_msg}, 404
    
    vinho.ano = new_vinho_year
    session.commit()
    logger.debug(f"Ano do vinho #{vinho_id} atualizado para '{new_vinho_year}'")
